...
export default function EditReviewPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/reviews/${params.id}`)
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch(() => { toast.error("Failed to load review"); setLoading(false); });
  }, [params.id]);
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Update client review</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-xl border bg-card p-6">
        ...
        <div className="space-y-2">
          ...
          <Input id="external_url" type="url" value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)} />
        </div>
